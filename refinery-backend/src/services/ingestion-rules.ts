...
export async function executeRule(id: string): Promise<{ filesFound: number; filesIngested: number; skipped: number }> {
  ...
  // Skip duplicates — check against already-ingested source keys
  let alreadyIngested = new Set<string>();
  if (rule.skip_duplicates) {
    const existing = await query<{ source_key: string }>(`SELECT DISTINCT source_key FROM ingestion_jobs WHERE status = 'complete'`);
