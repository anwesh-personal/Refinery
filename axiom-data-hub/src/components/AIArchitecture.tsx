...
const initialNodes: Node<NodeData>[] = [
  ...
  { id: 'ch', type: 'data', position: { x: 60, y: 0 }, data: { label: 'ClickHouse', subtitle: 'Universal Person DB', color: 'var(--text-tertiary)', icon: '💾', layer: 'data', details: '**ClickHouse** is your core analytical database. It stores every lead you\'ve ever ingested — first name, last name, email, company, job title, phone, LinkedIn, and 50+ more columns.\n\nAll pipeline steps read from and write to this database.\n\n**Tables:** `universal_person` (main), plus segment materialized views.\n\n**Performance:** Sub-second queries across millions of rows.' } },
  { id: 'sb', type: 'data', position: { x: 350, y: 0 }, data: { label: 'Supabase', subtitle: 'Auth, Config, AI Memory', color: 'var(--text-tertiary)', icon: '🗄️', layer: 'data', details: '**Supabase** handles everything non-analytical:\n\n- **User auth** — login, roles, permissions\n- **Server configs** — ClickHouse/S3 connections\n- **AI provider settings** — API keys, models\n- **Agent definitions** — prompts, KB, settings\n- **Conversation history** — chat memory\n- **Usage tracking** — token counts, costs\n\nAll frontend CRUD goes through Supabase.' } },
  { id: 's3', type: 'data', position: { x: 630, y: 0 }, data: { label: 'S3 / MinIO', subtitle: 'Raw data files', color: 'var(--text-tertiary)', icon: '☁️', layer: 'data', details: '**S3-compatible storage** holds your raw data files:\n\n- CSV/TSV uploads from data providers\n- Parquet files for bulk ingestion\n- Export files (target lists)\n\nIngestion pulls files from S3 → transforms → loads into ClickHouse.' } },
...
const initialEdges: Edge[] = [
  ...
  // Pipeline flow (L→R)
  { id: 'e-ing-merge', source: 'ingest', target: 'merge', animated: true, style: { stroke: 'var(--purple)', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: 'var(--purple)' } },
  { id: 'e-merge-seg', source: 'merge', target: 'segment', animated: true, style: { stroke: 'var(--yellow)', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: 'var(--yellow)' } },
  { id: 'e-seg-ver', source: 'segment', target: 'verify', animated: true, style: { stroke: 'var(--red)', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: 'var(--red)' } },
  { id: 'e-seg-tgt', source: 'segment', target: 'target', sourceHandle: 'bot', type: 'smoothstep', style: { stroke: 'var(--yellow)', strokeWidth: 2 }, markerEnd: { type: MarkerType.ArrowClosed, color: 'var(--yellow)' } },
  { id: 'e-ver-tgt', source: 'verify', target: 'target', sourceHandle: 'bot', type: 'smoothstep', style: { stroke: 'var(--yellow)' } },
  ...
  { id: 'e-seg-nx', source: 'segment', target: 'nexus', sourceHandle: 'bot', type: 'smoothstep', style: { stroke: 'var(--yellow)' } },
  { id: 'e-ver-nx', source: 'verify', target: 'nexus', sourceHandle: 'bot', type: 'smoothstep', style: { stroke: 'var(--red)' } },
...
// ═══ LAYERS LEGEND ═══
const LAYERS = [
  { key: 'data', label: 'Data Stores', color: 'var(--text-tertiary)', emoji: '💾' },
    ...
    <div style={{ position: 'relative' }}>
      ...
      <div style={{ height: 'calc(100vh - 320px)', borderRadius: 16, overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--bg-app)' }}>
        ...
        >
          ...
          <MiniMap
            nodeColor={(n) => (n.data as NodeData)?.color || 'var(--text-tertiary)'}
